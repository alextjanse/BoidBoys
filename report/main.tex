...
The visualization is implemented in JavaScript using a rendering stack based on Three.js, which internally utilizes WebGL. Three.js provides comprehensive camera management, geometry, and lighting, enabling the use of higher-level rendering code. We employed instanced rendering, where each boid is represented by a simple cone geometry. Furthermore, we added two rectangles to the sides of each cone to simulate flapping wings by having them move continuously in a synchronized manner. The transformation matrices for these instances are computed on the GPU.
