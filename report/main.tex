...
utulization of the GPU for both the simulation and visualization of the model.
...
Some limitations we ran into were the difficulty of debugging. Computation
...
The visualisation is implemented in Javascript, where our rendering stack is
...
geometry. The transformation matrices are computed on the GPU and applied per
instance. One of the advantages of using Three.js is the ease of use for rapid
testing and development. Because it abstraact low-level graphics programming,
setting up a camera, lighting and basic geometry was easy to do.
...
The behaviour of the boids is based on the boids paper by Craig
Reynolds~\cite{reynolds1987flocks}. This section describes the algorithm in
general, the modifications we made, and the choice of parameters.

\subsection{The model}\label{subsec:model}

The main model consists of a combination of the following 3 steering
behaviours: seperation, alignment and cohesion. Seperation aims to make sure
boids do not collide or crowd too much. A force is applied opposite to
neighbouring boids, where closer boids are considered heavier. Alignment is
...
boids. Combining these 3 steering behaviours with different influence strength
results in a realistic flock of birds. Finding the neighbouring birds is based
on a radius around the bird, as well as a maximum angle. \todo{Have we
    implemented the vision angle, or just the radius?} This is used to emulate the
vision of a bird, any boids outside the range are ignored.
...
input has been gathered, the current state of the simulation is send to the
GPU\@. Table~\ref{tab:bindings} displays the data that is send to the GPU\@.
Note that this is required information for the basic simulation, and does not
include the data needed for any acceleration structure. Chapter~\ref{sec:accel}
...
cpu and rendered. The rendering is done as an instancedmesh, which means that
all the boids are combined into a single mesh, and drawn as a single draw call.
This is possible, because all boids use the same mesh as a base, and only a
rotation and translation is applied. The usage of an instancedmesh results in
faster rendering times.

\subsection{Our modifications}\label{subsec:modifications}
While the simulation started of as an exact implementation of the boids paper,
we have made some modifications. One of the modifications is the interaction
with the mouse cursor. We have created a system where the boids flee from a
line emitted from the cursor. This system works by casting a ray from the mouse
...
important that the distance is not performed on every boid in the simulation.
...
partitions world space; each boid is mapped to a cell and cells store linked
...
(the surrounding $3 \times 3 \times 3$ cells).
...
involve boids leaving behind some form of "pheromone" or marker in the
...
communication between boids. \bigbreak The way Stigmergy is implemented in Mark
Tensen's research \cite{order_of_magnitude_boids} is though a texture based
approach. Which allows for boids to keep a global interactivity intact
thoughout the simulation. Because using Spatial Hashing like we do
\ref{subsec:spatial_hash} only allows for local interactivity, meaning that
boids can only interact with other boids in their local neighbourhood. This is
because the Spatial Hashing approach relies on a grid-based partitioning of
space, which limits the interactions to nearby cells.
...
The core of this optimization is the implementation of a parallel Spatial Hash
grid. The process is broken down into the following steps on the GPU:
\begin{itemize}
...
On the rendering side, we could have improved the visualisation by implementing
...
without seperating out the mesh instances into groups based on the camera
...
located. This artifact caused the boids to cluster heavily in that area,
...
Coming into this project with a background in WebGPU, Three.js, and flocking
algorithms, I felt very comfortable stepping into the technical implementation
from day one. My primary focus was the simulation's architecture, specifically
implementing the spatial hashing logic required for GPU-side neighbor lookups
at scale. To validate our goal of 100,000+ agents, I developed a benchmarking
tool that moved us beyond anecdotal observations by generating 1\% low FPS
metrics and automated LaTeX tables. I also spent significant time researching a
Stigmergy-based system for global interactivity; while this remained
experimental due to visual artifacts and time constraints, it provided crucial
insights into our grid's limitations.

The team's workflow was exceptionally efficient, largely because we established
a working MVP early in the cycle. This allowed us to spend the latter half of
the project focusing on polish and new features rather than basic
functionality. I found it particularly rewarding that the group possessed such
high programming fluency, which ensured that merging features and
troubleshooting across the codebase was a low-friction, collaborative
experience.
...
implementation I have contributed to the spatial hashing implementation, and
created the mouse interaction. Besides that, I have helped others when we
worked together on the programming. On the report I have mostly written the
project description, software section, and the simulation chapter.
