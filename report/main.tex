...
before giving the results we have achieved.
...
testing and development. Because Three.js abstracts low-level graphics programming, setting up a camera, lighting and basic geometry were easy to do.

One of the primary advantages of using Three.js is the ease of use it offers for rapid testing and development. By abstracting low-level graphics programming, the framework made setting up the camera, lighting, and basic geometry straightforward to implement.  We could potentially improve the visualization further by implementing Level of Detail (LOD) systems or frustum culling optimizations. These additions would likely increase the rendering speed of the simulation. However, the exact performance gain is currently unknown, as the rendering component is not explicitly profiled at this time.
...
The boids form a graph $G=(V,E)$, where $V$ represents the boids and $E$ contains edges between boids
that are within a viewing distance $R$ of each other. This graph is updated at each simulation tick
by updating the positions in $V$ and adjusting the neighbor relationships in $E$. Edges that persist
or are removed from $E$ are straightforward to update: simply check the new distances and update
accordingly. The main complexity lies in identifying new edges.

At every frame, the boids must perform a fixed-radius nearest neighbor search to determine their new
neighbors. Usually, spatial hashing is used, but we have also analyzed the geometric properties
of the boids in an attempt to optimize the simulation differently. Unfortunately, these findings did
not result in a practical solution, but are still worth mentioning.

A sweep surface algorithm can be used to scan over data points in lexicographical order (e.g., top
to bottom). By keeping track of the active boids -- those passed and  within distance $R$ of the sweep
surface -- we can significantly reduce the search space for neighboring boids. This space can be
reduced even further by performing spatial queries on the active set, limiting comparisons to boids
within a $2R \times 2R \times 2R$ query box around the boid of interest.

To support this, we would need a spatial data structure capable of efficient insertions, deletions,
and spatial queries. In two dimensions, the sweep state is one-dimensional and can be efficiently
maintained using a binary search tree. In higher dimensions, a range tree or $k$-d tree could be used.
However, both structures have costly insertion and deletion operations, and maintaining balance is
non-trivial. Additionally, these structures rely on pointers, making them less suitable for GPU
execution. Since spatial hashing also reduces the search space and performs well on GPUs, we opted
for that approach.
...
\subsection{Boid count limit}
\label{subsec:count_limit}
Due to the size of GPU buffers and how our CPU $\leftrightarrow$ GPU pipeline, the maximum amount of boids that our simulation can render is 1,295,419. For this reason, we are unable to fully test the limits of our program on a high-end GPU (RTX 5070). In order to allow more agents in our simulation, we could have split the array which stores their data into separate position, velocity and directions arrays. Although reporting the limits of our program could be interesting, our initial goal was to be able to work with 100K agents, so we did not find it necessary to go further than 10 times that number.
...
We ran our benchmarking tool (described in Section \ref{subsec:benchmark}) on a system using an AMD Ryzen 9 9950X CPU and an Asus Prime GeForce RTX 5070 GPU. We ran the benchmarking tool with 100K boids, and utilizing the default parameters of the program for the simulation. This was ran with the blank background setting. The results can be seen in Table \ref{tab:boid-benchmark-report-100k-standard}. Our main goal of maintaining a stable 60FPS was achieved, and we see a $1\%$ Low FPS of 59.2, which shows that the achieved FPS displays very little stuttering. For further investigation, we also ran the benchmarking with 500K and 963K\footnote{Although the maximum number of boids is 1,295,419 (Section \ref{subsec:count_limit}), the benchmarking tool stops rendering the boids at 963,203, which leads to wrong measurements since the program is then only a blank screen.} boids, and in both runs we got average FPS values of approximately 60. However, we see that the stuttering in these is a lot more drastic, with $1\%$ Low FPS of almost two-thirds of the average for the former, and almost one-third for the latter. Nevertheless, we still see a mostly stable performance at 60FPS with numbers of boids much greater than what we originally set for our goal.The tables for these runs can be found in Appendix \ref{appendix:tables}.

\hspace{5.0ex}

For comparison, we also ran the benchmarking tool on a much less powerful system, which uses a 13th Gen Intel Core i5-1335U CPU and Intel Iris Xe Graphics integrated GPU, utilizing the same parameters as previously described; the results can be seen in Table \ref{tab:boid-benchmark-report-100k-weak}. We also ran the other two tests on this system, which can be found in the \ref{appendix:tables}. For the 100K run, we see a very noticeable decrease in performance, with the average FPS dropping to 55.1, and with FPS values as low as 40.6. This is expected as this system does not have a dedicated graphics card, but it still shows that, perhaps, the program could be even further optimized to work on lower-end machines. For the higher number of boids, we see a significant drop in FPS, which is, again, expected. These results are being displayed purely to contrast the above described performance with a low-specs system and to entertain the idea that the program could be further optimized to display better performance in such systems.
...
Metric & Value \\
\hline
Benchmark &  100K Boids\\
Boid Count & 100000 \\
Separation Weight & 1.50 \\
Alignment Weight & 1.00 \\
Cohesion Weight & 0.50 \\
Max Speed & 5.00 \\
Update Frequency & 60 \\
Avg Simulation Time (ms) & 0.03 \\
Avg Render Time (ms) & 0.28 \\
Avg Frame Time (ms) & 16.68 \\
Avg FPS & 60.0 \\
1\% Low Frame Time (ms) & 16.90 \\
1\% Low FPS & 59.2 \\
CPU & AMD Ryzen 9 9950X \\
GPU & Asus Prime GeForce RTX 5070 GDDR7 12GB\\
\hline
\end{tabular}
\caption{Boid benchmark report: 100K boids}
\label{tab:boid-benchmark-report-100k-standard}
\end{table}
...
Metric & Value \\
\hline
Benchmark & 100K boids with lower specs \\
Boid Count & 100000 \\
Separation Weight & 1.50 \\
Alignment Weight & 1.00 \\
Cohesion Weight & 0.50 \\
Max Speed & 5.00 \\
Update Frequency & 60 \\
Avg Simulation Time (ms) & 0.11 \\
Avg Render Time (ms) & 0.84 \\
Avg Frame Time (ms) & 18.16 \\
Avg FPS & 55.1 \\
1\% Low Frame Time (ms) & 24.64 \\
1\% Low FPS & 40.6 \\
CPU & 13th Gen Intel Core i5-1335U \\
GPU & Intel Iris Xe Graphics \\
\hline
\end{tabular}
\caption{Boid benchmark report: 100K boids with lower specs}
\label{tab:boid-benchmark-report-100k-weak}
\end{table}
...
I entered the project with a strong foundation in WebGPU, Three.js, and flocking algorithms, which
allowed me to hit the ground running on the core architecture from the very first week. My primary
goal was to ensure the simulation could scale efficiently to meet our ambitious performance targets.
To achieve this, I focused heavily on the technical implementation of the spatial hashing logic, which
was critical for moving the computational burden of neighbor detection onto the GPU. To validate our
progress, I also developed a comprehensive benchmarking and reporting system. This tool allowed us
to move beyond anecdotal observations by capturing standardized metrics, such as 1\% low FPS counts
and automated LaTeX tables, providing a data-driven backbone for our results section. I also dedicated
significant time to researching and prototyping a Stigmergy-based system for global interactivity.
Although this remained in the not implemented in the final solution due to time constraints and specific behavioral
artifacts where boids would cluster at the edges of grid slices. The research was very helpfull for
understanding the limitations and synchronization challenges of our spatial grid.

Beyond the initial implementation, I served as the primary debugger for our WGSL shaders, navigating
the complexities of parallel programming to resolve technical hurdles that are often difficult to
track in a web environment. Reflecting on the project as a whole, I was incredibly impressed by our
team's efficiency. We managed to produce a functional "MVP" very early in the cycle, which significantly
reduced the typical end-of-project pressure and gave us the freedom to focus on high-level optimizations
and feature expansion. I found it particularly rewarding that every member of the group possessed a
high level of general programming fluency. This shared technical competence made merging diverse
feature branches and troubleshooting across the codebase a low-friction, collaborative experience.
The synergy between our different perspectives, combined with an open line of communication and a
collective eagerness to learn, made this one of the most positive team experiences I have had. I am
very proud of the cohesive, high-performance system we were able to build together.

\subsection*{Alex Janse}
% Alex's contribution and self-reflection

When I started this project, I already had some experience in game and web development, but my main
strength lies in algorithmic analysis and design. During my studies, I followed the Geometric Algorithms
(INFOGA) course, and I tried to apply several of the techniques from that course to our simulation.
In addition, I explored different neighborhood search algorithms to investigate whether further
optimizations were possible.

Within the project, I contributed to setting up the base project structure and worked on parts of the
user interface. This gave me a good balance between working on both the theoretical and practical
aspects of the system.

One thing I appreciated was that we completed the minimal requirements relatively early in the process.
This gave us the freedom to experiment with alternative approaches and explore optimizations without
depending on uncertain research outcomes.

Looking back, the collaboration within the team was fine, but it was mostly working individually on
seperate parts. I liked that it gave me the freedom to focus on my own interest within the project,
but I also feel like we could have helped each other out more. On the other, because of each individual's
contribution, the project has interesting results from many different approaches. Overall, I'm happy
with the results.
...
experience with GPU programming. For what was relevant for this project, I had only previously worked
directly with GLSL in the implementation of vertex and fragment shaders. For this reason, I felt that
I wasn't able to do much at the beginning, while the team set up the project, as I still needed to
learn the basics. At first, I found myself most useful during our group meetings, when we mostly
discussed the overall details of the project, rather than direct implementations, as I was able to
find literature on the relevant topics and discuss our goals for the project. 

The group was very efficient during the first couple of weeks, meaning that we already had a simulation
running and a functional UI. My main contribution during this part was connecting the modifiable
parameters in the UI to the WebGPU pipeline. Further along the line, I attempted implementing an
approximate nearest-neighbours search in the GPU \cite{leite2012nearest} in order to optimize the
efficiency of the program, but it turned out to not have any meaningful impact on our results. I also
experimented with different shapes for the bounding box (e.g. sphere, torus) and added the EXR
background to the program, instead of a blank one. Lastly, I implemented a functionality to change
vary the color of each boid based on its starting position, as a way to observe how the starting
position of boids affected their position and behaviour further into the simulation. I was also
responsible for setting up the report and bibliography, and proof-reading.

I believe I could have been more helpful to the team had I communicated better early on in the project
and asked for help earlier in how to implement certain systems, as I spent a lot of my time trying
to understand the inner-workings of the program until I was able to attempt implementing things. I
felt overwhelmed at first, since everyone in the group seemed very knowledgeable of the tools and
knew where to start, and once I started properly communicating (about 1.5 to 2 weeks in) most of the
basic functionalities had already been implemented. I still feel I was helpful and could contribute
during group meetings, and I believe I understand our implementation very well and could reproduce it
given enough time, but looking back made me realize I could have started being helpful earlier on.
...
Max Speed & 5.00 \\
Update Frequency & 60 \\
Avg Simulation Time (ms) & 0.05 \\
Avg Render Time (ms) & 1.36 \\
Avg Frame Time (ms) & 16.68 \\
Avg FPS & 59.9 \\
1\% Low Frame Time (ms) & 21.52 \\
1\% Low FPS & 46.5 \\
CPU & AMD Ryzen 9 9950X\\
GPU & Asus Prime GeForce RTX 5070 GDDR7 12GB\\
\hline
\end{tabular}
\caption{Boid benchmark report: 500K Boids}
\label{tab:boid-benchmark-report-500K-standard}
\end{table}
...
Metric & Value \\
\hline
Benchmark & Maximum number of boids \\
Boid Count & 963203 \\
Separation Weight & 1.50 \\
Alignment Weight & 1.00 \\
Cohesion Weight & 0.50 \\
Max Speed & 5.00 \\
Update Frequency & 60 \\
Avg Simulation Time (ms) & 0.04 \\
Avg Render Time (ms) & 3.01 \\
Avg Frame Time (ms) & 16.67 \\
Avg FPS & 60.0 \\
1\% Low Frame Time (ms) & 43.76 \\
1\% Low FPS & 22.9 \\
CPU & AMD Ryzen 9 9950X \\
GPU & Asus Prime GeForce RTX 5070 GDDR7 12GB\\
\hline
\end{tabular}
\caption{Boid benchmark report: Maximum number of boids}
\label{tab:boid-benchmark-report-max-standard}
\end{table}
...
Metric & Value \\
\hline
Benchmark & 500K boids with lower specs \\
Boid Count & 500000 \\
Separation Weight & 1.50 \\
Alignment Weight & 1.00 \\
Cohesion Weight & 0.50 \\
Max Speed & 5.00 \\
Update Frequency & 60 \\
Avg Simulation Time (ms) & 0.07 \\
Avg Render Time (ms) & 3.62 \\
Avg Frame Time (ms) & 62.88 \\
Avg FPS & 15.9 \\
1\% Low Frame Time (ms) & 130.70 \\
1\% Low FPS & 7.7 \\
CPU & 13th Gen Intel Core i5-1335U \\
GPU & Intel Iris Xe Graphics \\
\hline
\end{tabular}
\caption{Boid benchmark report: 500K boids with lower specs}
\label{tab:boid-benchmark-report-500K-weak}
\end{table}
...
Metric & Value \\
\hline
Benchmark & Maximum number of boids with lower specs \\
Boid Count & 963203 \\
Separation Weight & 1.50 \\
Alignment Weight & 1.00 \\
Cohesion Weight & 0.50 \\
Max Speed & 5.00 \\
Update Frequency & 60 \\
Avg Simulation Time (ms) & 0.09 \\
Avg Render Time (ms) & 5.71 \\
Avg Frame Time (ms) & 108.38 \\
Avg FPS & 9.2 \\
1\% Low Frame Time (ms) & 343.60 \\
1\% Low FPS & 2.9 \\
CPU & 13th Gen Intel Core i5-1335U \\
GPU & Intel Iris Xe Graphics \\
\hline
\end{tabular}
\caption{Boid benchmark report: Maximum number of boids with lower specs}
\label{tab:boid-benchmark-report-max-weak}
\end{table}
