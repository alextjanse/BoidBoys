...
Another modification is creating a `soft boundary`. To ensure the boids remain
in the scene, but do not create an obvious bounding box shape, we have created
a soft boundary. This boundary is implemented by adding a small force back to
the center, when boids exit the bounding box. The boids can still exit, but are
pushed back slightly, and even more when getting further away. This resulted in
