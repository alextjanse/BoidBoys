...
testing and development. \hl{Because it abstraact low-level graphics programming},
...
communication between boids. \bigbreak \hl{HIGHLIGHT START} The way Stigmergy is implemented in Tensen's research \cite{order_of_magnitude_boids} is through a texture based approach, which allows for boids to keep a global interactivity intact throughout the simulation. Because using Spatial Hashing as described in \ref{subsec:spatial_hash} only allows for local interactivity, meaning that boids can only interact with other boids in their local neighbourhood. This is because the Spatial Hashing approach relies on a grid-based partitioning of space, which limits the interactions to nearby cells.

\hl{HIGHLIGHT END}

\subsubsection*{What has been done}
To scale up our flocking simulation, we integrated concepts from Tensen's work directly into our WebGPU pipeline. \hl{Rather than calculating boid behaviors in nearby cells, we.}
...
\medbreak
By implementing Tensen's GPU-driven spatial hashing \cite{order_of_magnitude_boids}, we localized the nearest-neighbour search, vastly reducing the number of distance calculations required per frame. Furthermore, this keeps the entire pipeline strictly on the GPU, which eliminates the massive performance overhead of transferring hundreds of thousands of vector states back and forth between CPU and GPU memory.
\medbreak
...
The reason as to why we haven't implemented this in our final program is because of the issues we had with the implementation. Implementing this caused the boids to be attracted to a slice of the space where the grid cells were located, which caused the boids to cluster together in that area and not be able to move freely in the space. We tried to fix this issue by adjusting the parameters of the spatial hash, but we couldn't find a solution that worked well enough. This made it impossible for us to determine if the implementation actually helped resolve the issue of global interactivity, which is the main reason why we haven't included it in our final program.
